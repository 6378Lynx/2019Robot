...
public class ShoulderSubsystem extends Subsystem implements Loggable {
  ...
  private CheesyPID pid = new CheesyPID(0.15, 0, 0.02, 0);
  ...
  private double feedforward_voltage() {
    ...
    double kf = RobotMap.armMass * 9.8 * Math.cos(Math.toRadians(getPos()))
        * RobotMap.centreOfMass * ffGain;
  ...
  public void setDegrees(double angle, double motion_feedforward) {
    ...
    shoulder.set(pid.calculate(this.getPos(), prevTime == Double.NaN ? 0.02 : currentTime - prevTime)
        + feedforward_voltage() + (motion_feedforward/12 * 12/RobotMap.getVoltageComp));
    prevTime = currentTime;
  }

  public void holdPosition(double input) {
    if(Math.abs(input)<0.15){
      input = 0;
    }
    angle = angle + (input * 0.25);
    currentTime = Timer.getFPGATimestamp();
    pid.setSetpoint(angle);
