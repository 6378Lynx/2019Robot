...
//import lynxlib.drive.LynxDrive;
...
public class DriveSubsystem extends Subsystem implements Loggable {
  ...
  public DriveSubsystem(){
    ...
    drive.setEncoders(leftEncoder, rightEncoder);
    drive.setVelocityPIDs(leftPID, rightPID);
    drive.setFeedForwardConstants(RobotMap.drive_kv, RobotMap.Vintercept);

    drive.setGyroStabilizationEnabled(false);
  ...
  public void teleopDrive(double frontSpeed, double sideSpeed){
    ...
    if(Robot.oi.controller.getBumper(GenericHID.Hand.kRight)){
      drive.curvatureDrive(frontSpeed/4.5, sideSpeed/4.5, Robot.oi.controller.getBumper(GenericHID.Hand.kRight));
    }
